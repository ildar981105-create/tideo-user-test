---
name: tideo-user-test
description: "This skill provides a complete user testing toolkit for web applications, including automatic behavior tracking (tracker.js), contextual in-flow surveys (survey.js), SUS/EV/NPS standard questionnaires, task panels, and SAM self-assessment models. Use this skill when the user mentions: user testing, usability testing, behavior tracking,埋点, 问卷调查, SUS评分, 可用性测试, 用户研究, A/B test preparation, or needs to collect real user interaction data on any web product."
---
...
TideoTracker.milestone('signup_done');   // 注册完成 → 触发 T1 任务 + 对应 checkpoint
TideoTracker.milestone('step2_done');    // 下单完成
TideoTracker.milestone('task_done');     // 支付完成 → 触发 T3 + 任务结束 + 总结问卷
```
...
| `tasks` | array | Tideo 默认 | 任务定义数组（见下方格式） |
| `checkpoints` | array | Tideo 默认 | 情境问卷数组（见下方格式） |
| `welcomeInternal` | object | Tideo 默认 | 内部测试欢迎页文案 |
| `welcomeUser` | object | Tideo 默认 | 外部用户欢迎页文案 |
| `testVideoUrl` | string | `'assets/test-video.mov'` | 测试素材下载地址 |
| `openQuestion1/2` | string | Tideo 默认 | 总结问卷开放题文本 |
...
```javascript
// 底层
TideoTracker.record(type, detail)   // 记录任意事件
TideoTracker.flush()                 // 强制刷新缓冲区

// 里程碑（最常用）
TideoTracker.milestone(name)          // 记录 milestone（自动映射）
TideoTracker.milestone(name, extra)   // 带附加数据

// 阶段
TideoTracker.phaseStart(name)
TideoTracker.phaseEnd(name)

// 上传
TideoTracker.uploadProgress(pct)      // 0-100
TideoTracker.uploadDone(url, sizeMB)
TideoTracker.uploadFail(err)

// MPS 状态（保留给 Tideo 特定场景）
TideoTracker.mpsStatus(status, elapsed)

// 问卷联动（Survey 内部使用）
TideoTracker.surveyShow(id, type)
TideoTracker.surveyAnswer(data)
TideoTracker.surveySkip(id)
TideoTracker.surveyExitSubmit(data)
TideoTracker.taskComplete(taskId, label)

// 信息
TideoTracker.uid        // 当前用户 ID
TideoTracker.testRound  // 当前测试轮次
TideoTracker.sessionId  // 当前会话 ID
```
...
```bash
cd ~/.codebuddy/skills/tideo-user-test
git init
git add .
git commit -m "feat: tideo-user-test v3 configurable"
git remote add origin https://github.com/YOUR_USERNAME/tideo-user-test.git
git push -u origin main
```

其他人使用：
```bash
git clone https://github.com/YOUR_USERNAME/tideo-user-test.git
# 或作为 submodule
```
